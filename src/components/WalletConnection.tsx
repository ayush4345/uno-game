...
export function WalletConnection({ onConnect }: WalletConnectionProps) {
      ...
      if (address && connector) {
        try {
          ...
          if (connector.id === 'base' || connector.id === 'coinbaseWallet') {
            ...
            const provider = await connector.getProvider() as BaseAccountProvider;
  ...
  const handleSelectSubAccount = (subAccountAddress: string) => {
    ...
    toast({
      ...
      description: `Using subaccount ${subAccountAddress.substring(0, 6)}...${subAccountAddress.substring(subAccountAddress.length - 4)}`,
    });
  };
          ...
          <WalletDropdown className="z-50">
            ...
            {subAccounts.length > 0 && (
              <div className="border-t border-gray-200 py-2 px-4">
                <p className="text-sm font-medium text-gray-500 mb-2">Subaccounts</p>
                {subAccounts.map((subAccount) => (
                  <button
                    key={subAccount}
                    onClick={() => handleSelectSubAccount(subAccount)}
                    className={`w-full text-left py-2 px-2 rounded-md text-sm ${selectedAccount === subAccount ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
                  >
                    {subAccount.substring(0, 6)}...{subAccount.substring(subAccount.length - 4)}
                    {selectedAccount === subAccount && (
                      <span className="ml-2 text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded-full">Active</span>
                    )}
                  </button>
                ))}
              </div>
